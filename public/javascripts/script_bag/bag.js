...
if (bag == null || bag.length == 0)
{
    ...
    <div id="content-img" class="d-flex align-items-end d-flex justify-content-center">
        ...
        <p>Gio hang cua ban con trong</p>
    </div><hr>
...
else 
{
    ...
    for (let i = 0; i < product.length; i++)
    {
        if (number[i] != 0)
        {
            ...
            {
                if (product[i].hasOwnProperty(key)) {
                    str += key + ' ' + product[i][key] + '.';
                }
            }
            ...
            `
                <div class="card" style="width: 18rem;" id="${i}">
                    ...
                    <div class="card-body">
                        ...
                        <button type="button" class="btn btn-outline-danger" onclick="removeItem(${i})">Danger</button>
                        <input type="number" value="${number[i]}" min="0" max="${product[i].count}" onchange="changeNumber(${i+1000})" id="${i+1000}">
                    </div>
                </div>
            `;
        }
